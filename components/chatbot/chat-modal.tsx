...
export function ChatModal({ isOpen, onClose }: ChatModalProps) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hello! I'm your diabetes assistant. Type 'start assessment' to check your diabetes risk, or 'find doctors in [location]' to search for specialists. You can also ask any question about diabetes.",
    },
  ]);
  ...
  const getGeminiResponse = async (question: string, context?: string) => {
    try {
      ...
      if (context === "welcome") {
        prompt =
          "You are a friendly Diabetes Chatbot. Introduce yourself briefly and mention that users can type 'start assessment' to check their diabetes risk, 'find doctors in [location]' to search for specialists, or ask questions about diabetes. Keep it under 3 sentences.";
      ...
      if (lowerInput === "start assessment") {
        ...
        if (!location) {
          setMessages((prev) => [
            ...
            {
              role: "assistant",
              content: "Please specify a location (e.g., 'find doctors in Mumbai' or 'search doctors near Delhi').",
            },
          ]);
          setIsLoading(false);
          return;
    ...
    if (nextStep < questions.length) {
      ...
      try {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: "I've prepared a detailed report for you that you can download below. If you'd like, I can also help you find diabetes specialists near your location—just type 'find doctors in [location]'!",
          },
        ]);
